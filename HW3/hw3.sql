CREATE TABLE product (maker VARCHAR(20), model VARCHAR(20), type VARCHAR(8) DEFAULT '' NOT NULL, CHECK(type='pc' OR type='laptop' OR type='printer' OR type='smartphone' OR type='tablet'), PRIMARY KEY (model));

CREATE TABLE pc (model VARCHAR(20), speed FLOAT(4), ram INT(8), hd INT(8), price INT(4), PRIMARY KEY (model), FOREIGN KEY (model) REFERENCES product(model));

CREATE TABLE laptop (model VARCHAR(20), speed FLOAT(4), ram INT(8), hd INT(8), screen VARCHAR(20), price INT(4), PRIMARY KEY (model), FOREIGN KEY (model) REFERENCES product(model));

CREATE TABLE printer (model VARCHAR(20), color VARCHAR(20), type VARCHAR(8), price INT(4), PRIMARY KEY (model), FOREIGN KEY (model) REFERENCES product(model));
